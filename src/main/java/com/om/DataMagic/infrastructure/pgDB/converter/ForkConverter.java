...
public class ForkConverter {
    ...
    public ForkDO toDO(JsonNode repoJson,String owner,String repo,String codePlatForm){
        ...
        if (null!=dateString){
            ...
            forkDO.setCreatedAt(Date.from(zonedDateTime.toInstant()));
        }

        String updatedAt = repoJson.get("updated_at")==null?null:repoJson.get("updated_at").asText();
        if (null!=updatedAt){
            ZonedDateTime zonedDateTime = ZonedDateTime.parse(updatedAt, DateTimeFormatter.ISO_OFFSET_DATE_TIME);
            forkDO.setUpdatedAt(Date.from(zonedDateTime.toInstant()));
        }
        ...
        if (null!=pushedAt){
            ...
            forkDO.setPushedAt(Date.from(zonedDateTime.toInstant()));
        }
